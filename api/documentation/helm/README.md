# contactcatalogus chart

The Helm chart installs contactcatalogus and by default the following dependencies using subcharts:
...
```bash
helm repo add contactcatalogus https://raw.githubusercontent.com/ConductionNL/contactcatalogus/master/api/helm/
helm repo update
```
...
```bash
helm install my-contactcatalogus ConductionNL/contactcatalogus --version 0.1.0
```
...
| `settings.subdomain` | the subdomain of the installation excluding www. | `contactcatalogus` |
| `settings.subpath` | Any sub path to follow the domain, like /api/v1 | `contactcatalogus` |
